...
const App = () => {
  const [userId, setUserId] = useState(null); // Store userId in App state
  const [token, setToken] = useState(null);   // Store token in App state
  ...
  return (
    <Router>
      <Routes>
        ...
        {/* Pass userId and token to Dashboard */}
        <Route 
          path="/dashboard" element={<Dashboard userId={userId} token={token} />} />
        {/* Other routes */}
