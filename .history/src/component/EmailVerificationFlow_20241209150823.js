...
import { verifyOtp, resendOtp } from "../Api Folder/verifyOtp";
...
const EmailVerificationFlow = () => {
  ...
  const [showOtpErrorToast, setShowOtpErrorToast] = useState(false);
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const response = await verifyOtp(corpId, email, otpValue);
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const response = await resendOtp(corpId, email);
      ...
      <div className="row credential">
        <div className="col-lg-6 bg-primary text-white bg-icons">
          ...
          <div className="wrapper-inner mt-5">
            ...
            >
              <h2 className="text-center text-primary fs-3 fw-bold">
                Verify OTP
              </h2>
              ...
              {otpError && (
                <Toast
                  onClose={() => setShowOtpErrorToast(false)}
                  show={showOtpErrorToast}
                  delay={3000}
                  autohide
                  bg="danger"
                >
                  <Toast.Body>{otpError}</Toast.Body>
                </Toast>
              )}
              ...
              <Form onSubmit={handleVerifyOtp}>
                ...
                <Button
                  type="submit"
                  className="mb-1 rounded-pill text-white"
                  disabled={loading}
