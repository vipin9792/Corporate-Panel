    ...
    const fetchPlansData = async () => {
      try {
        const response = await axios.post(
          ...
          {
            headers: {
              Authorization: `Bearer YOUR_VALID_TOKEN`  // Ensure this token is valid
            }
          }
        );
        ...
        // Handle error case if API returns error response
        if (response.data.code && response.data.status) {
          if (response.data.code === 1001) {
            console.error(`API Error: ${response.data.status}`);
            alert(`API Error: ${response.data.status}`);  // Show the error to the user
            return;  // Exit early if the response contains an error
          }
        }
  
        // Handle success case where the response contains plans
        if (Array.isArray(response.data.plans)) {
          setPlansData(response.data.plans);  // Successfully set the plans data
        } else {
          console.error('Expected an array of plans, but got:', response.data);
          setPlansData([]);  // Set plansData to an empty array if the response is not as expected
        }
                    ...
                    <div className="col-md-4 mb-4" key={plan.id}>
                      <div className="card" style={{
                        ...
                        <div className="card-footer" style={{
                          ...
                          <Link to={`/UpgradePlan/${corp_id}`} className="btn btn-primary">Choose Plan</Link>
                        </div>
                      </div>
                    </div>
                  ))}
