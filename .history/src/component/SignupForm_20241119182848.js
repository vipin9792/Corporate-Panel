  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.corp_id) {
        ...
        // Navigate to OTP verification page with corpId and email as query parameters
        const email = values.emailId;
        navigate(`/verify-otp?corpId=${response.data.corp_id}&email=${email}`);
      } else {
                        ...
                        <div className="col-lg-10 mx-auto">
                          <div className="form-floating mb-3">
                            ...
                            <label htmlFor="companyName">
                              <span><img src="name.svg" alt="" /></span> Company Name
                            </label>
                            <ErrorMessage name="companyName" component="div" className="error-message" />
                          </div>
                        </div>

                        {/* Other fields for Email, Phone, Address, etc. */}
                        {/* Email, PhoneNo, Address, UserId, Password, Full Name Fields */}
