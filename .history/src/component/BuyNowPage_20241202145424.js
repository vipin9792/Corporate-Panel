...
const BuyNowPage = () => {
  const { corp_d, id_plan } = useParams(); // Extract the 'id_plan' parameter
  const [planDetails, setPlanDetails] = useState(null); // Store plan details from API

  // Fetch the plan details using the API
  useEffect(() => {
    const fetchPlanDetails = async () => {
      try {
        const response = await axios.get(`http://103.35.121.219:4000/corp/subscription/fetchPlanDetails?corp_d=${corp_d}&plan_id=${id_plan}`);
  ...
  }, [corp_d, id_plan]); // Trigger re-fetch when 'corp_d' or 'id_plan' changes

  if (!planDetails) {
    return <div>Loading...</div>; // Show loading while fetching the data
  }
                ...
                <div className="col-md-6">
                  ...
                  <div className="mt-4">
                    ...
                    <select className="form-select" id="colorSelect">
                      {/* Replace with the study materials related to this plan */}
