...
const PlanDetails = () => {
  const { corp_id, id_plan } = useParams(); // Get corp_id and plan id from URL params
  const [planData, setPlanData] = useState(null); // Store plan details
  const [loading, setLoading] = useState(false); // Loading state
  const [error, setError] = useState(null); // Error state
  const [razorpayOrderId, setRazorpayOrderId] = useState(null); // Store Razorpay Order ID

  const BEARER_TOKEN = "your_valid_bearer_token_here"; // Replace with your valid bearer token
  ...
  const handleBuyNow = async () => {
    try {
      ...
      if (response.data.code === 1000) {
        const { razorpay_payment_id, sdk_key } = response.data;

        setRazorpayOrderId(response.data.pay_detail.order_id); // Save Razorpay Order ID

        const options = {
          key: sdk_key, // Razorpay API key
          amount: response.data.pay_detail.amount * 100, // Amount in paisa (1 INR = 100 paisa)
          currency: "INR",
          name: "Your Company Name",
          description: "Subscription Plan Payment",
          order_id: response.data.pay_detail.order_id,
          handler: async function (response) {
            const razorpayPaymentId = response.razorpay_payment_id;
            // After successful payment, update the payment status via your API
            await updatePaymentStatus(razorpayPaymentId, response.order_id);
          },
          prefill: {
            name: "User TP", // Prefill user information
            email: "user@example.com",
            contact: "1234567890",
          },
          theme: {
            color: "#528FF0", // Customize Razorpay theme
          },
        };
