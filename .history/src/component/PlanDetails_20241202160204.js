...
const PlanDetails = () => {
  const { plan_ref, corp_id } = useParams();  // Extract plan_ref and corp_id from the URL
  ...
  const fetchPlanDetails = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { plan_ref, corp_id },  // Pass both plan_ref and corp_id to fetch details
        {
          headers: {
  ...
  useEffect(() => {
    if (!plan_ref || !corp_id) {
      alert('Plan ref or Corp ID is missing!');
      console.log('Error: plan_ref or corp_id is undefined');
    } else {
      alert(`Fetching details for plan: ${plan_ref} with Corp ID: ${corp_id}`);
  ...
  }, [plan_ref, corp_id]);  // Dependency on plan_ref and corp_id
