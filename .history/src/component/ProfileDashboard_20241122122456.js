...
const ProfileDashboard = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        // Adding console logs to see request flow
        ...
        console.log('API Response:', response);  // Log the full API response for better visibility
        ...
        if (response.data.code === 1000) {
          ...
          alert('API Response Error: ' + response.data.status); // Alert the error from the response
        }
      } catch (err) {
        setError('Error fetching data');
        console.error('API Fetch Error:', err); // Log the specific error from axios request
