import React, { useState, useEffect } from "react";
import axios from "axios";

const ViewProfile = ({ corp_id }) => {
    const [profile, setProfile] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState("");

    useEffect(() => {
        const fetchProfileData = async () => {
            setLoading(true);
            try {
                // Make API request to fetch profile data
                const response = await axios.post(
                    "http://103.35.121.219:4000/corp/dashboard/fetchProfile",
                    { corp_id: corp_id },
                    {
                        headers: {
                            Authorization: `Bearer YOUR_TOKEN_HERE`, // Ensure your token is correct
                        },
                    }
                );

                // Log the full response and profile data to inspect
                console.log("API Response:", response);
                console.log("Profile Data:", response.data.profile); // Check the profile data here

                // Check if the profile data exists and has a valid structure
                if (response.data.code === 1000 && response.data.profile) {
                    setProfile(response.data.profile); // Set profile if valid
                    setError(""); // Clear any previous errors
                } else {
                    setError("Failed to load profile. Please try again.");
                }
            } catch (err) {
                console.error("Error fetching profile:", err); // Log any errors
                setError("An error occurred while fetching the profile.");
            } finally {
                setLoading(false); // Set loading to false when done
            }
        };

        // Fetch the profile when component is mounted or when corp_id changes
        fetchProfileData();
    }, [corp_id]);

    // Render profile or error message
    return (
        <div className="container mt-3">
            {loading ? (
                <div>Loading profile...</div> // Show loading message
            ) : error ? (
                <div className="alert alert-danger">{error}</div> // Show error message
            ) : (
                profile && (
                    <div className="profile-details">
                        <div className="row mb-4">
                            <div className="col-lg-6">
                                <h3>Profile Information</h3>
                                <div className="card">
                                    <img
                                        src={profile.logo}
                                        alt="Company Logo"
                                        className="card-img-top"
                                        style={{
                                            width: "100%",
                                            height: "200px",
                                            objectFit: "cover",
                                        }}
                                    />
                                    <div className="card-body">
                                        <h5 className="card-title">{profile.company_name || "No Company Name"}</h5>
                                        <p className="card-text">
                                            <strong>Email: </strong>
                                            {profile.email || "No Email Provided"}
                                        </p>
                                        <p className="card-text">
                                            <strong>Phone: </strong>
                                            {profile.phone_no || "No Phone Provided"}
                                        </p>
                                        <p className="card-text">
                                            <strong>Address: </strong>
                                            {profile.address || "No Address Provided"}
                                        </p>
                                        <p className="card-text">
                                            <strong>Username: </strong>
                                            {profile.username || "No Username Provided"}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="row">
                            <div className="col-lg-6">
                                <div className="card">
                                    <div className="card-body">
                                        <h5 className="card-title">Storage Used</h5>
                                        <p className="card-text">
                                            <strong>Storage: </strong>
                                            {profile.storage_used ? profile.storage_used : "No Data"} MB
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                )
            )}
        </div>
    );
};

export default ViewProfile;
