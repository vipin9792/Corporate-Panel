...
const LoginForm = () => {
  ...
  const [toastMessage, setToastMessage] = useState(""); // Holds message for toast
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (response.data.code === 1000) {
        ...
        const corpId = response.data.profile.id; // Assuming ID is available for navigation
  ...
  return (
    <div>
      ...
      {showToast && (
        <div
          className={`toast align-items-center text-white ${toastType === "success" ? "bg-success" : "bg-danger"} position-fixed bottom-0 end-0 p-3`}
          role="alert"
          aria-live="assertive"
          aria-atomic="true"
          style={{ zIndex: 9999 }}
