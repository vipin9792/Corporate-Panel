...
const LoginForm = () => {
  ...
  const [corpId, setCorpId] = useState(null); // Store the corp_id for OTP process
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (response.data.code === 1000) {
        // Store corp_id in state to use in OTP flow
        setCorpId(response.data.profile.id); // Assume `id` is the correct `corp_id`

        setMessage('Login successful!');
        alert('Login Successful!');
        
        // Check if the email is verified before proceeding
        if (response.data.profile.verify_status === 0) {
          setError('Email not verified yet. Please verify your email first.');
          // Call the OTP resend function here
          handleResendOTP(response.data.profile.email);
        } else {
    ...
    try {
      if (corpId) {
        const response = await axios.post(
          ...
          {
            corp_id: corpId, // Use the stored corp_id
            email: email
          },
          {
            headers: {
        ...
        setError('No corp_id available for OTP request.');
      }
    } catch (err) {
