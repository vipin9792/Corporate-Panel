...
const Dashboard = ({ userId, token }) => {
  ...
  // If token or userId is missing, redirect to the login page
  useEffect(() => {
    if (!token || !userId) {
    ...
    try {
      const response = await axios.get(
        `http://103.35.121.219:4000/corp/dashboard/fetchProfile`,
        {
          ...
          params: {
            corp_id: userId,  // Use the userId in the request params
          },
        }
      );
      
      if (response.data.code === 1000) {
        setProfileData(response.data.profile);  // Set the profile data in state
      } else {
