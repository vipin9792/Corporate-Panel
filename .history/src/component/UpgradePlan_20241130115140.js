...
const UpgradePlan = () => {
    ...
    useEffect(() => {
        ...
        const fetchPlansData = async () => {
            try {
                // Make API call with the bearer token for authentication
                const response = await axios.post('http://103.35.121.219:4000/corp/subscription/fetchAllPlans/plans', {
                    headers: {
                        Authorization: `Bearer !WCZVSa3eTg!rE8#0GjAhulUyvOuu!TKB1vz$Y!baEak&u.ZMvikJ3OrjwvcGe91H5@nNY200EkYECpOYRNbu25aipgUGK0Ebi6HuNUDZsV$.MaKIikX6sg!9FaQnJcz`
                    }
                });
                setPlansData(response.data);  // Assuming the response structure is an array of plans
                setLoading(false);
            } catch (error) {
