...
const Dashboard = () => {
  ...
  const fetchCorpId = async () => {
    try {
      // Example API call to fetch corpId (Replace with your actual API logic)
      const response = await fetch('http://103.35.121.219:4000/corp/dashboard/fetchProfile');  // Your actual endpoint here
      ...
      if (data && data.corpId) {
        ...
        // Handle error or missing corpId
  ...
  // Debugging to check if corpId is available
                  ...
                  <div className="collapse" id="collapseProfile">
                    <ul>
                      <li className="menu-item mt-2">
                        {/* Check if corpId exists and then navigate */}
                        {corpId ? (
                          ...
                          <span>Loading...</span>  // This should only show if corpId is still loading
                        )}
                      </li>
