  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch Transaction Data (for plan purchase)
        ...
        if (transactionResponse.data && transactionResponse.data.transactions.length > 0) {
          const latestTransaction = transactionResponse.data.transactions[0]; // Latest transaction
          setPlan(latestTransaction);  // Set plan data from the most recent transaction
        } else {
          setPlan(null); // No transactions, set plan to null
          setError("No active plan found. Please purchase a plan to view your subscription details.");
        }
                ...
                <div className="container ">
                  {plan ? (
                    ...
                    >
                      ...
                      <Link
                        to={`/UpgradePlan/${corp_id}`}to={`/UpgradePlan/${corp_id}`}
