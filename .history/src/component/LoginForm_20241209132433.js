...
const LoginForm = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    console.log("Form Values:", values); // Debugging line to see the form values
    try {
      const responseData = await loginApi(values);
      console.log("API Response:", responseData); // Debugging line to log API response

      if (responseData.code === 1000) {
        setMessage("Login successful!");
        
        // Using toast notification instead of alert
        toast.danger("Login Successful!");
