...
const StudentListPage = () => {
  ...
  const [batches, setBatches] = useState([]); // New state to store batches
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [studentsPerPage] = useState(5); // Number of students per page

  // Fetch students and batches data when the component is mounted or when corp_id changes
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (studentsResponse.data.code === 1000) {
          setStudents(studentsResponse.data.students);
        } else {
          setError("Unexpected response code: " + studentsResponse.data.code);
        }

        // Fetching batches data
        const batchesResponse = await axios.post(
        ...
        if (batchesResponse.data.code === 1000) {
          setBatches(batchesResponse.data.batches);
        } else {
          setError("Unexpected response code: " + batchesResponse.data.code);
        }
      } catch (err) {
                ...
                <li className="menu-item">
                  <Link to={`/TransectionPage/${corp_id}`} className="menu-link text-decoration-none">
                    <FontAwesomeIcon
                      ...
                      style={{ color: "white" }}
                    />
                    <span className="mx-2 text-white"> Transaction</span>
                  </Link>
                </li>
