...
const EmailVerificationFlow = () => {
  ...
  const [canResendOtp, setCanResendOtp] = useState(true); // To control resend button enable/disable state
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const response = await axios.post(
        ...
        { corp_id, email }, // Pass the correct data
        {
          headers: {
            Authorization: `Bearer !WCZVSa3eTg!rE8#0GjAhulUyvOuu!TKB1vz$Y!baEak&u.ZMvikJ3OrjwvcGe91H5@nNY200EkYECpOYRNbu25aipgUGK0Ebi6HuNUDZsV$.MaKIikX6sg!9FaQnJcz`
          }
      ...
      if (response.data.code === 1000) {
        ...
        // Start the 50-second countdown when OTP is resent
        setTimer(50); // Start the countdown from 50 seconds
  ...
  useEffect(() => {
    if (timer > 0) {
      ...
      return () => clearInterval(interval); // Cleanup the interval when component unmounts or timer changes
    }
  }, [timer]);
