...
const EditProfile1 = () => {
  ...
  const [apiRequestSent, setApiRequestSent] = useState(false); // Track if API request is sent
  ...
  const handleSubmit = (e) => {
    ...
    setError("");
    setSuccess("");
    setApiRequestSent(true); // Mark the request as sent

    // Log profile data before submitting
...
`, // Replace with your token
      ...
      .then((response) => {
        ...
        if (response.data.code === 1000) {
          ...
          // Wait for a few seconds before redirecting to ensure backend has processed the update
          setTimeout(() => {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn btn-primary mt-3" disabled={apiRequestSent}>
          Save Changes
        </button>
      </form>
    </div>
