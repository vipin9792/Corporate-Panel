...
const LoginForm = () => {
  ...
  const [corpId, setCorpId] = useState(null);  // Add a state for corpId
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (response.data.code === 1000) {
        ...
        setCorpId(response.data.corp_id); // Set corp_id dynamically based on the login response
  ...
  const handleResendOTP = async (email) => {
    if (!corpId || !email) {
      setResendMessage('Missing corpId or email. Please ensure these are set correctly.');
      return;
    }
                  ...
                  {({ isSubmitting, values }) => (
                    <Form>
                      ...
                      <div className="col-lg-10 mx-auto">
                        <button
                          type="button"
                          onClick={() => handleResendOTP(values.email)} // Pass `values.email` here
