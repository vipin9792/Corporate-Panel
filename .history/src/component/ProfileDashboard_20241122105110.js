...
const ProfileDashboard = () => {
  ...
  const BEARER_TOKEN = token;  // Use the token from the URL directly
  ...
  useEffect(() => {
    if (!token || !corpId) {
      ...
      navigate('/login');  // Redirect to login if the token or corpId is missing
      return;
    }
    ...
    fetchProfileData();
  }, [token, corpId, BEARER_TOKEN, navigate]); // Only run when token or corpId changes
