    ...
    const fetchData = async () => {
      try {
        ...
        if (transactions.length === 0) {
          ...
          setPlan(null); // No transactions found, so no plan to show
        } else {
          ...
          if (successfulTransactions.length > 0) {
            ...
            setPlan(null); // No successful transactions found
          }
        }
      } catch (err) {
                ...
                <div className="container">
                  {plan ? (
                    ...
                    <div className="alert alert-warning text-center p-4 shadow-sm rounded">
                      ...
                      <Link to={`/UpgradePlan/${corp_id}`} className="btn btn-info">
                        Upgrade Now
                      </Link>
                    </div>
                  )}
                      ...
                      >
                        <div className="card-body d-flex align-items-center justify-content-between">
                          <div className="flex-grow-1 text-start">
                            ...
                            <h5 className="card-title my-2 text-white fs-5">
                              Total Batches
                            </h5>
                          </div>
                          <h5 className="fw-bold text-white fs-1">{data.batch}</h5>
                        </div>
                      </span>
                    </div>
                    {/* Other stats */}
