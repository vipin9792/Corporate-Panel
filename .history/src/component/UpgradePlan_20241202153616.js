...
const UpgradePlan = () => {
  ...
  useEffect(() => {
    const fetchPlanDetails = async () => {
      ...
      try {
        // Corrected the method to axios.get
        const response = await axios.get(`http://103.35.121.219:4000/corp/subscription/fetchPlanDetails?corp_id=${corp_id}&plan_id=${id_plan}`, {
          headers: {
        ...
        if (response.data.plan) {
          ...
          throw new Error("Plan details not found in response");
        }
      } catch (error) {
  ...
  }, [corp_id, id_plan]); // Fetch plan when 'corp_id' or 'id_plan' changes
  ...
  if (!planDetails) {
    return <div>No plan details available.</div>; // Show if no plan data is available
  }
