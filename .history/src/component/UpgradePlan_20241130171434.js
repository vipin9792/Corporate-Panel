...
const UpgradePlan = () => {
  ...
  const refreshToken = async () => {
    try {
      const storedRefreshToken = localStorage.getItem('refresh_token'); // Replace with actual token storage
      if (!storedRefreshToken) {
        throw new Error('No refresh token found. Please log in again.');
      }

      const response = await axios.post('http://your-api.com/refresh', { token: storedRefreshToken });
      const newToken = response.data.new_token;

      localStorage.setItem('auth_token', newToken); // Store the new token
      return newToken;
    } catch (error) {
  ...
  const fetchPlansData = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token'); // Get the current token
      ...
      const response = await axios.post(
        ...
        {
          headers: {
            Authorization: `Bearer ${token}` // Use the token from localStorage or state
          }
        }
      );
      ...
      if (response.data.code === 1000) {
        ...
        // Token error, try to refresh or prompt for login
        const newToken = await refreshToken();
        // Retry the fetch with the new token
        await fetchPlansData(); // Retry the function with the new token
      } else {
