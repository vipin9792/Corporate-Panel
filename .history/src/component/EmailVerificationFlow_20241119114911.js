  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (response.data.code === 1000) {
        ...
        navigate('/login');
        console.log("OTP verified successfully:", response.data); // Log success
      } else {
        setError('Invalid OTP or email verification failed.');
        console.log("OTP verification failed:", response.data); // Log failure
      }
    } catch (err) {
      setError('An error occurred during OTP verification.');
      console.error("Error during OTP verification:", err); // Log error
    }
  };
