...
const EmailVerificationFlow = () => {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    // Check if corp_id and otp are available before proceeding
    if (!corp_id || !otpValue) {
    ...
    try {
      const response = await axios.post(
        ...
        { corp_id, otp: otpValue }, // Send corp_id and OTP
        {
          headers: {
  ...
  const handleResendOtp = async () => {
    ...
    // Check if corp_id and email are available
    if (!corp_id || !email) {
    ...
    try {
      const response = await axios.post(
        ...
        { corp_id, email }, // Send corp_id and email
        {
          headers: {
            Authorization: `Bearer !WCZVSa3eTg!rE8#0GjAhulUyvOuu!TKB1vz$Y!baEak&u.ZMvikJ3OrjwvcGe91H5@nNY200EkYECpOYRNbu25aipgUGK0Ebi6HuNUDZsV$.MaKIikX6sg!9FaQnJcz`
          }
