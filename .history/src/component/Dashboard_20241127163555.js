...
const Dashboard = () => {
  ...
  const fetchCorpId = async () => {
    try {
      ...
      const response = await fetch('/api/getCorpId');  // Replace with your actual endpoint here
      
      if (!response.ok) {
        // If the response is not OK, throw an error
        throw new Error(`Failed to fetch corpId: ${response.statusText}`);
      }
      ...
      setLoading(false); // Ensure loading state is disabled after the API call
    }
  };
