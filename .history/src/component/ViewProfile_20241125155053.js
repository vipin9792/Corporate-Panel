...
const ViewProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchCorpId = async () => {
      try {
        // Make the request to fetch corp_id
        ...
        // Log the response to check if the API is working fine
        console.log('API Response:', response.data);

        if (response.data.code === 1000) {
          const corpId = response.data.profile.id; // Accessing the corp_id from the response
          console.log('Fetched corp_id:', corpId); // Log the corp_id to verify
    ...
    // Fetch profile data from the API using the corp_id
    const fetchProfileData = async (corpId) => {
      ...
      try {
        ...
        console.log('Profile Data:', response.data); // Log to check profile data structure

        if (response.data.code === 1000) {
          // If profile data is fetched successfully, update the state
    ...
    fetchCorpId(); // Fetch the corp_id when the component mounts
