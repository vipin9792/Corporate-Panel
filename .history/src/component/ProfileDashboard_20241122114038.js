...
const ProfileDashboard = () => {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem('auth_token'); // Get the Bearer token from localStorage

    if (!corpId || !token) {
      setError('No company ID or token found. Please log in again.');
      alert('No company ID or token found. Please log in again.'); // Alert for missing corp_id or token
      return;
    }
