...
const EmailVerificationFlow = () => {
  ...
  // Ensure that email and corp_id are in localStorage on mount
  useEffect(() => {
    const storedEmail = localStorage.getItem('email');
    const storedCorpId = localStorage.getItem('corp_id');

    // Check if email or corp_id are missing in localStorage
    if (!storedEmail || !storedCorpId) {
      setError('Corp ID or Email is missing in localStorage. Please try again.');
      console.log("Error: Email or Corp ID missing!");
    } else {
      console.log("Email and Corp ID found:", storedEmail, storedCorpId);
    }
  }, []);
