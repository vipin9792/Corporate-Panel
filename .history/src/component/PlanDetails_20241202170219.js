...
const PlanDetails = () => {
  ...
  useEffect(() => {
    const fetchPlanDetails = async () => {
      ...
      console.log("Plan Ref:", plan_ref); // Log plan_ref
      console.log("Corp ID:", corp_id); // Log corp_id

      try {
        const response = await axios.post(
          'http://103.35.121.219:4000/corp/subscription/fetchPlanDetails',
          { plan_ref, corp_id }, // Send data to the API
          {
            headers: {
    ...
    if (plan_ref && corp_id) {
      fetchPlanDetails(); // Fetch data when plan_ref and corp_id are available
    }
  }, [plan_ref, corp_id]);
