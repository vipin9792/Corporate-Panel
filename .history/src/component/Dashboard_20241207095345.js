  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Transaction Response:", transactionResponse.data);  // Log the full response to debug
  
        // Filter successful transactions
        const successfulTransactions = transactionResponse.data.transactions.filter(
          (transaction) => transaction.payment_status === 1  // Make sure this matches the actual value for success
        );
        
        console.log("Filtered Successful Transactions:", successfulTransactions); // Log the filtered transactions
