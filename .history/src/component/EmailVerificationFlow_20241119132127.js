...
const EmailVerificationFlow = () => {
  ...
  const handleResendOtp = async () => {
    ...
    const email = localStorage.getItem('email');  // Ensure email is in localStorage

    console.log('Resend OTP - Corp ID:', corp_id);  // Debugging: Check the corp_id
    console.log('Resend OTP - Email:', email);  // Debugging: Check the email

    // Ensure both corp_id and email exist before making the request
    if (!corp_id || !email) {
      setError('Corp ID or email is missing. Please try again.');
      setIsResending(false);
      return;  // Don't proceed if either is missing
    }

    try {
      const response = await axios.post(
        'http://103.35.121.219:4000/corp/resendOTP', // API to resend OTP
        { corp_id, email },  // Send corp_id and email to the API
        {
          headers: {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    console.log('Verify OTP - Corp ID:', corp_id);  // Debugging
    console.log('Verify OTP - OTP:', otpValue);  // Debugging

    // Ensure both corp_id and otpValue exist before making the request
    if (!corp_id || !otpValue) {
