...
const UpgradePlan = () => {
  const { corp_id, plan_id } = useParams(); // Get both corp_id and plan_id from URL params
  ...
  const fetchPlansData = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { corp_id, plan_id }, // Pass both corp_id and plan_id from URL
        {
          headers: {
  ...
  useEffect(() => {
    if (!corp_id || !plan_id) {
      alert("corp_id or plan_id is missing from the URL!");
    } else {
