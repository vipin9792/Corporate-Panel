...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Check if the response code is 1000 (success)
      if (response.data.code === 1000) {
        setData(response.data.detail);  // Set the data if successful
      } else {
        console.error("Unexpected response code:", response.data.code);
        setError("Unexpected response code: " + response.data.code);
      }
    } catch (err) {
      // Log and set error message if something goes wrong
        ...
        <div className="layout-container" style={{ position: "relative" }}>
          ...
          <div className="layout-page bg-white">
            ...
            <div className="container mt-1">
              ...
              {/* Show the fetched data if it exists */}
              {data && (
                <div className="alert" role="alert" style={{ background: "#EDF2FF", borderRadius: "10px" }}>
                  ...
                  <Link to="/UpgradePlan" className="btn btn text-white" style={{ background: "#1C4481" }}>
                    Upgrade Plan
                  </Link>
                </div>
              )}
