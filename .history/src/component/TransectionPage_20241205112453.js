...
const TransactionPage = () => {
  const { transaction_id } = useParams(); // Transaction ID from URL parameters
  ...
  useEffect(() => {
    const fetchTransactionDetails = async () => {
      ...
      try {
        console.log("Fetching transaction details for ID:", transaction_id); // Debugging log
        alert("Fetching transaction details..."); // Alert before fetching
        ...
        alert(`API Response: ${JSON.stringify(response.data)}`); // Alert the API response
        ...
        if (response.data.code === 1000) {
          ...
          alert(`Error: ${response.data.status || "Unknown error"}`); // Alert error
        }
      } catch (err) {
        console.error("Error fetching transaction details:", err);
        setError("An error occurred while fetching the transaction details.");
        alert(`Error: An error occurred while fetching the transaction details.`); // Alert error
      } finally {
        setLoading(false);
      }
    };
