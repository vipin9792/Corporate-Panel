...
import { useForm } from "../FormContext"; // Import useForm hook
...
const Dashboard = () => {
  ...
  const { formData } = useForm(); // Access form data, including corp_id
  const navigate = useNavigate();

  useEffect(() => {
    if (!formData.corpId) {
      // Redirect if corpId is not available
      navigate("/signup");
    }
  }, [formData.corpId, navigate]);
  return (
    <>
