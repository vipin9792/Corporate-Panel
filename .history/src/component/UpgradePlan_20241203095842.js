...
const UpgradePlan1 = () => {
  ...
  const handleUpgrade = async (planRef, corpId) => {
    try {
      // Make an API call to upgrade the plan
      const response = await axios.post(
        "http://103.35.121.219:4000/corp/subscription/upgradePlan", // Sample API URL for upgrading a plan
        {
          corp_id: corpId,
          plan_ref: planRef, // The plan reference ID
        },
        {
          headers: {
            Authorization: `Bearer !WCZVSa3eTg!rE8#0GjAhulUyvOuu!TKB1vz$Y!baEak&u.ZMvikJ3OrjwvcGe91H5@nNY200EkYECpOYRNbu25aipgUGK0Ebi6HuNUDZsV$.MaKIikX6sg!9FaQnJcz`,
      ...
      if (response.data.code === 1000) {
        // If upgrade was successful, navigate to the plan details page
        navigate(`/plan-details/${planRef}/${corpId}`);
      } else {
        // Handle errors or unsuccessful upgrades
                            ...
                            <div className="card shadow-sm" style={{ height: "420px" }}>
                              <div className="card-body">
                                ...
                                <button
                                  ...
                                  onClick={() => handleUpgrade(plan.plan_ref, corp_id)}
                                >
