...
const UpgradePlan = () => {
  ...
  try {
    ...
    console.log('API Response:', response.data); // Log the whole response to check its structure
    ...
    if (error.response) {
      ...
      alert(`Failed to fetch plans data. Server responded with status ${error.response.status}`);
    } else if (error.request) {
      // If no response was received
      console.error('No response received:', error.request);
      alert('Failed to fetch plans data. No response from server.');
    } else {
      // General errors
            ...
            <div className="layout-page bg-white">
              ...
              <div className="container mt-5">
                <div className="row text-center">
                  {plansData.length > 0 && plansData.map((plan) => (
                    <div className="col-md-4 mb-4" key={plan.id}>
                      <div className="card" style={{
                        ...
                        <div className="card-body" style={{ padding: "25px", background: "#f8f9fa", borderRadius: "10px" }}>
