...
const SignUp = () => {
  ...
  useEffect(() => {
    <axios className="post"></axios>('http://103.35.121.219:4000/init/getPhotoSlider')
      .then((response) => {
        // Assuming the API returns an array of slides
            ...
            <div className="warapper-form-alert carouselWrapper" style={{ marginTop: "50px" }}>
              <div id="carouselExample" className="carousel slide" data-bs-ride="carousel" style={{ width: '400px', margin: 'auto', borderRadius: '24px' }}>
                <div className="carousel-inner">
                  {/* Render dynamic carousel items based on fetched slider data */}
                  {sliderData.length > 0 ? (
                    sliderData.map((slide, index) => (
                      <div key={index} className={`carousel-item ${index === 0 ? 'active' : ''}`}>
                        {/* <img src={slide.imageUrl} className="d-block w-100" alt={`Slide ${index + 1}`} /> */}
                        <img src={slide.photos.photo_url} className="d-block w-100" alt={`Slide ${index + 1}`} />
