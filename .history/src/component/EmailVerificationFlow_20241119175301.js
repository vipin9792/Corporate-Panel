...
const EmailVerificationFlow = () => {
  ...
  const [corpId, setCorpId] = useState(null); // Initialize corpId state
  ...
  useEffect(() => {
    // Set the corpId from query params if available
    if (registeredCorpId) {
      setCorpId(registeredCorpId);
    }

    // For debugging: check if the corpId and email are correctly set
    console.log('corpId from query params:', registeredCorpId);
    console.log('Email:', email);
  }, [registeredCorpId, email]);
