...
const UpgradePlan = () => {
  ...
  const fetchPlansData = async () => {
    ...
    try {
      // Make sure to replace with your correct API key here
      const response = await axios.post(
        'http://103.35.121.219:4000/corp/subscription/fetchAllPlans',
        {}, // Empty body, as shown in your example
        {
          headers: {
      ...
      // Process the plans data if valid
          ...
          <div className="plans-container">
            {plansData.map((plan) => (
              <div key={plan.id} className="plan-card">
                ...
                <p><strong>Total Cost:</strong> ${plan.total_cost}</p>
