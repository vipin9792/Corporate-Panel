...
const SignupForm = () => {
  ...
  const [corpId, setCorpId] = useState(null); // Added state to store corp_id
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.corp_id) {
        ...
        setCorpId(response.data.corp_id); // Store corp_id in state

        // Redirect to OTP verification page with corpId as a query parameter
      ...
      <section>
        <div className="row credential">
          ...
          <div className="col-lg-6">
            <Slider /> {/* Assuming this is your slider component */}
          </div>
        </div>
      </section>
    </div>
