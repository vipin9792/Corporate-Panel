...
",
        ...
        console.log("Dashboard Data:", dashboardResponse.data);  // Log full response for debugging
        if (dashboardResponse.data.code === 1000) {
          setData(dashboardResponse.data.detail);  // Update dashboard data
        } else {
          setError("Unexpected response from dashboard API");
        }
  
        // 2. Fetch Transaction Data for Plan Purchase
        ...
        console.log("Transaction Data:", transactionResponse.data);  // Log full transaction data
  
        // 3. Check if any transactions exist
        if (transactionResponse.data && transactionResponse.data.transactions.length > 0) {
          const transactions = transactionResponse.data.transactions;
          // Just use the latest transaction if present, no filtering needed
          const latestTransaction = transactions[transactions.length - 1];  // Get the most recent transaction
          setPlan(latestTransaction);  // Update the plan state with the latest transaction
        } else {
          setPlan(null);  // No transactions, set plan to null
          setError("No active plan found. Please purchase a plan to view your subscription details.");  // Update error message
        }
  ...
  }, [corp_id]);  // Run again when corp_id changes
