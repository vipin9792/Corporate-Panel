...
const SignupForm = () => {
  ...
  const handlePhoneNumberChange = (e) => {
    ...
    e.target.value = value.replace(/\D/g, '').slice(0, 10); // Only keep digits and limit to 10 characters
  };
  ...
  const validationSchema = Yup.object({
    ...
    phoneNo: Yup.string()
    .matches(/^[0-9]{10}$/, "Phone number must be exactly 10 digits and only numeric") // Ensures 10 digits only
    .required("Phone number is required"), // Makes phone number a required field
  ...
  <div className="form-floating mb-3">
    <Field
      ...
      onBlur={handleBlur}
      onInput={handlePhoneNumberChange} // Apply the change handler
      maxLength="10"  // Limits input to 10 characters
      inputMode="numeric" // Optimizes for numeric input on mobile
      pattern="[0-9]{10}" // Ensures only numbers are entered (regex is applied for validation)
