...
<div className="hero" style={{ background: "linear-gradient(to right, #4e54c8, #8f94fb)", color: "white", padding: "60px 0", textAlign: "center" }}>
    ...
    <div className="row text-center">
        {plansData.map(plan => (
            <div className="col-md-4 mb-4" key={plan.name}>
                <div className="card" style={{ background: `linear-gradient(135deg, ${plan.color} 30%, rgba(0, 0, 0, 0.9) 100%)`, border: "none", color: "white", transition: "transform 0.3s, box-shadow 0.3s", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5)" }}>
                    <div className="card-header" style={{ background: "rgba(0, 0, 0, 0.8)" }}>
                        <h3 style={{ color: "#FFD700" }}>{plan.name}</h3> {/* Gold color for better visibility */}
                    </div>
                    <div className="card-body">
                        <div style={{ fontSize: "50px", marginBottom: "15px" }}>
                            <FontAwesomeIcon icon={plan.icon} />
                        </div>
                        <h2 className="card-title" style={{ color: "#FFD700" }}>{isYearly ? plan.yearlyPrice : plan.monthlyPrice}</h2>
                        <p className="card-text" style={{ color: "#FFD700" }}>Features:</p>
                        <ul className="list-unstyled">
                            {plan.features.map((feature, index) => (
                                <li key={index} style={{ color: "#FFD700" }}>✔ {feature}</li>
                            ))}
                        </ul>
                    </div>
                    <div className="card-footer" style={{ background: "rgba(0, 0, 0, 0.8)" }}>
