...
const SignupForm = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      console.log(response.data);  // Log response to check if corp_id is returned
      if (response.status === 200) {
        setSuccessMessage("Signup successful! Please verify OTP.");

        // Extract corp_id from the response
        const corpId = response.data.corp_id;

        // If the corp_id is present, navigate to the view-profile page
        if (corpId) {
          navigate(`/view-profile?corpId=${corpId}`);
        } else {
          setError("No corpId found in the response.");
        }
      } else {
