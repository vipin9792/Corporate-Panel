  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (responseData.code === 1000) {
        ...
        toast.warning("Login Successful!");  // Replace alert with toast

        const corpId = responseData.profile.id;
        navigate(`/Dashboard/${corpId}`);
      } else {
        setError("Invalid credentials. Please try again.");
        toast.error("Login Failed! Invalid credentials.");  // Replace alert with toast
      }
    } catch (err) {
      console.error("Login Error:", err);
      setError("An error occurred during login.");
      toast.error("An error occurred during login.");  // Replace alert with toast
    }
    setSubmitting(false);
  };
