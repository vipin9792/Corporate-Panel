...
const Dashboard = () => {
  ...
  // Function to fetch userId dynamically (or from context or props)
  const fetchUserId = () => {
    // This is just an example; you might want to fetch the userId from context, props, or localStorage.
    const storedUserId = localStorage.getItem('userId');  // Example from localStorage
    if (storedUserId) {
      setUserId(storedUserId); // Set userId dynamically
    } else {
  ...
  useEffect(() => {
    ...
    fetchUserId(); // Call fetchUserId (or get userId from another source)
  }, []);

  // Show loading state until corpId and userId are fetched
  if (loading) {
    return <div>Loading...</div>;
  }

  console.log("Current corpId:", corpId);  // Debugging to check if corpId is available
  console.log("Current userId:", userId);  // Debugging to check if userId is available
                  ...
                  <div className="collapse" id="collapseProfile">
                    <ul>
                      <li className="menu-item mt-2">
                        {/* Check if corpId and userId exist */}
                        {corpId && userId ? (
                          <Link to={`/ViewProfile?corpId=${corpId}&userId=${userId}`} className="text-white text-decoration-none">
                            <FontAwesomeIcon icon={faUserEdit} size="1x" /> View Profile
                          </Link>
                        ) : (
                          <span>Loading...</span> // Display Loading... if corpId or userId is still null
                        )}
                      </li>
