...
import { useAuth } from "../component/AuthProvider"; // Import the custom AuthProvider hook
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (responseData.code === 1000) {
        ...
        const userData = {
          corpId: responseData.profile.id,
          name: responseData.profile.name,
          email: values.email,
        };
        login(userData); // Call the login function from AuthProvider

        // Redirect to the dashboard after a delay
        setTimeout(() => {
          navigate(`/Dashboard/${responseData.profile.id}`);
