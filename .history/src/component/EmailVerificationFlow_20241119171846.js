...
const EmailVerificationFlow = () => {
  ...
  const [resendLoading, setResendLoading] = useState(false);  // New state to handle resend loading
  const [email, setEmail] = useState('');
  const [corpId, setCorpId] = useState(null);  // State to store the corpId from registration
  const inputRefs = useRef([]);
  const location = useLocation();
  const navigate = useNavigate();

  const queryParams = new URLSearchParams(location.search);
  const registeredCorpId = queryParams.get('corpId');  // Extract corpId from URL params (if available)
  ...
  useEffect(() => {
    // If there's a registeredCorpId in the query parameters, set it in the state
    if (registeredCorpId) {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      // Ensure the corpId used here is from the registration response
      const response = await axios.post(
        'http://103.35.121.219:4000/corp/resendOTP',
        { corp_id: corpId, email }, // Use the correct corp_id and email
        {
          headers: {
            Authorization: `Bearer ${BEARER_TOKEN}`
          }
