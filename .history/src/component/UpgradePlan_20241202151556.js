...
const UpgradePlan = () => {
  ...
  useEffect(() => {
    const fetchPlanDetails = async () => {
      try {
        const response = await axios.post(`http://103.35.121.219:4000/corp/subscription/fetchPlanDetails?corp_id=${corp_id}&id_plan=${id_plan}`, {
          headers: {
  ...
  }, [corp_id, id_plan]); // Fetch plan when 'corp_d' or 'id_plan' changes
