...
const Dashboard = ({ onLogout }) => {
  ...
  useEffect(() => {
    // Get corpId from localStorage
    const storedCorpId = localStorage.getItem("corpId");

    if (!storedCorpId) {
      // If there's no corpId in localStorage, redirect to login
      navigate("/");
    } else {
      // Fetch user data if necessary
      setUserData({ corpId: storedCorpId });
    }
  }, [navigate]);

  const handleLogout = () => {
    // Handle logout: Clear the localStorage and redirect to the login page
