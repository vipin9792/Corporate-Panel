...
const EditProfile1 = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.code === 1000) {
          const { company_name, email, phone_no, address, username, name } = response.data.profile;
          setProfile({
            company_name,
            email,
            phone: phone_no,
            address,
            userid: username,
            name, // Added name field to the profile state
          });
        } else {
  ...
  const handleSaveChanges = () => {
    console.log("Updated profile data:", profile); // Debugging log
    ...
    axios
      ...
      .then((response) => {
        console.log("API Response on Save Changes:", response.data); // Debugging log
