...
const LoginPage = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (token) {
        // Store the token (e.g., in localStorage) and pass it to parent component
        localStorage.setItem("authToken", token); // Storing token for persistence
        onLoginSuccess(token); // Passing the token back to the parent component

        // Redirect to dashboard
        navigate("/dashboard");  // After successful login, redirect to dashboard
