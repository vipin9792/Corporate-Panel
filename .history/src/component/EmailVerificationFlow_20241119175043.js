...
const EmailVerificationFlow = () => {
  ...
  useEffect(() => {
    // Ensure corpId and email are correctly set
    if (registeredCorpId) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    // Ensure corpId and email are valid before making the API call
    if (!corpId || !email) {
      setOtpError('Missing corpId or email. Please ensure these are set correctly.');
      setLoading(false);
      return;
    }
  ...
  const handleResendOtp = async () => {
    ...
    console.log('Attempting to resend OTP with corp_id:', corpId); // Log for debugging
