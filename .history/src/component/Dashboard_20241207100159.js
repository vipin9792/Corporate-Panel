  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 3. Store transaction data for plan purchase
        if (transactionResponse.data && transactionResponse.data.transactions) {
          const transactions = transactionResponse.data.transactions;
  
          // Handle case if no transactions exist
          if (transactions.length === 0) {
            setError("No transactions found");
          } else {
            // Optionally, if you need the latest transaction, just grab it
            const latestTransaction = transactions[transactions.length - 1];  // Simple example to get the last transaction
