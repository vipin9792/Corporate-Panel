    ...
    const fetchPlansData = async () => {
      try {
        const response = await axios.post(
          ...
          {
            headers: {
              Authorization: `Bearer your-token-here`  // Make sure the token is correct
            }
          }
        );
        ...
        // Check if the response data is an array before using .map()
        if (Array.isArray(response.data)) {
          setPlansData(response.data);
        } else {
          console.error('Expected an array, but got:', response.data);
          setPlansData([]); // Set to empty array if data is not in expected format
        }
